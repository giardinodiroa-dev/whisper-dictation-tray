...
SUBCHUNK_TARGET_SECS = 4.0
SUBCHUNK_TARGET = int(SAMPLE_RATE / CHUNK_SAMPLES * SUBCHUNK_TARGET_SECS)

def _split_frames(frames):
    """Split a recorded frame buffer into ~4s phrase sub-chunks.

    Accumulates frames until SUBCHUNK_TARGET is reached, then splits
    at the next silent frame. Falls back to one chunk if no silence found.
    """
    if not frames:
        return [frames]
    sub_chunks = []
    current = []
    frames_since_split = 0
    for frame in frames:
        current.append(frame)
        frames_since_split += 1
        if frames_since_split >= SUBCHUNK_TARGET:
    ...
    if current:
        sub_chunks.append(current)
    return sub_chunks if sub_chunks else [frames]
...
def dictation_loop():
    ...
    while dictation_active:
        ...
        if frames:
            sub_chunks = _split_frames(frames)
            for sub in sub_chunks:
                seq = _seq; _seq += 1
                _inc_pending()
                _executor.submit(_process_chunk, seq, sub, win)
